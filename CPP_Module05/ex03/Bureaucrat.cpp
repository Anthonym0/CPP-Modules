...
void    Bureaucrat::sign_Form(bool sign, std::string const name)
{
    if (sign)
    {
...
void    Bureaucrat::executeForm(AForm const & form)
{
    if ( form.execute(*this) )
        std::cout << GREEN << this->getName() << RESET << " executed " << GREEN << form.getName() << RESET << std::endl;
    else
        std::cerr << RED << "ERROR: an error has occurred in the execution of the form" << RESET << std::endl;
}
